...
const UserAvatar = ({ user, size = 32, style }) => {
  const profilePic = user?.user_profile?.profile_picture ?? user?.profile_picture;
  const gender = user?.user_profile?.gender ?? user?.gender ?? '';

  const imgSrc = profilePic
    ? (profilePic.startsWith('http')
        ? profilePic
        : `${import.meta.env.VITE_API_BASE_URL}/storage/${profilePic}`)
    : fallbackSvg(gender);
